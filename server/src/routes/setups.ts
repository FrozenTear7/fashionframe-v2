...
router.get('/', csrfProtection, getSetups);
router.get('/user/:userId', [csrfProtection, validObjectId], getSetupsByUserId);
router.get('/favorites', [csrfProtection, auth], getUserFavorites);
router.get('/:id', [csrfProtection, validObjectId], getSetupById);
